 ...
 */
func (this *NetworkScanner) CheckAddress(driver *NetworkDriver, p1 int, p2 int) {
	// Address with format: "192.168.{p1}.{p2}:52133"
	addr := "192.168." + fmt.Sprint(p1) + "." + fmt.Sprint(p2) + ":52133"

	if system.IsStringInArray(addr, driver.AllConnectedAddresses()) {
		return
	}
 ...
 */
func (this *NetworkScanner) ScanLocal(driver *NetworkDriver) {
	this.availableAddresses = nil

	for i1 := 0; i1 < 16; i1++ {
		for i2 := 0; i2 < 256; i2++ {
			go this.CheckAddress(driver, i1, i2)
		}
